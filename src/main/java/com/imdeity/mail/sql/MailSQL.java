...
public class MailSQL {
    ...
    public static int getUnreadCount(String playername) {
        int    count = 0;
        String sql   = "SELECT COUNT(*) FROM " + Mail.database.tableName("mail") + " WHERE `receiver` = '" + playername
                     + "' AND `read` = '0';";
    ...
    public static boolean sendMail(String sender, String receiver, String message) throws SQLException {
        String sql = "INSERT INTO " + Mail.database.tableName("mail") + " (" + "`sender`, `receiver`, `message`"
                     + ") values (?,?,?);";
    ...
    public static void getAllMail(Player player) {
        String sql = "SELECT id, sender, receiver, message FROM " + Mail.database.tableName("mail")
                     + " WHERE `receiver` = '" + player.getName().toLowerCase() + "' AND `read` = '0';";
        HashMap<Integer, ArrayList<String>> result = Mail.database.Read(sql);
        ...
        if (result.isEmpty()) {
            ...
            for (int i = 1; i <= result.size(); i++) {
                ...
                MailObject mail     = new MailObject(id, index, sender, receiver, message);
    ...
    public static void getAllMail(Player player, String playername) {
        ...
        if (result.isEmpty()) {
            Mail.mail.sendPlayerMessage(player, Language.getInboxEmptyOther().replaceAll("%player", playername));
            Mail.mail.sendPlayerMessage(player, Language.getMailErrorHelp());

            return;
        } else {
            Mail.mail.sendPlayerMessage(player, "&c--[" + playername + "'s Inbox]--");
    ...
    private static MailObject getMail(String player, int offset) {
        String sql = "SELECT @rownum:=@rownum+1 AS rownum, id, sender, message, send_date FROM "
                     + Mail.database.tableName("mail") + ", (SELECT @rownum:=0) r WHERE `receiver` = '" + player
                     + "' AND `read` = '0';";
        ...
        if ((result != null) &&!result.isEmpty() && (result.size() >= offset)) {
            ...
            MailObject mail = new MailObject(id, index, sender, receiver, message, sendDate);
    ...
    public static void setClosedMail(Player player, int id) throws SQLException {
        ...
        String sql = "UPDATE " + Mail.database.tableName("mail") + " SET `read` = '1' WHERE id = '" + tmpMail.getId()
                     + "';";

        Mail.database.Write(sql);

        for (String line :
                tmpMail.preformReplacement(Language.getMailDeleteSingular().replaceAll("%messageNumber",
                    tmpMail.getIndex() + ""))) {
    ...
    public static void setAllClosedMail(Player player) throws SQLException {
        String sql = "UPDATE " + Mail.database.tableName("mail") + " SET `read` = '1' WHERE `receiver` = '"
                     + player.getName() + "';";
            ...
            if (unread > 0) {
                if (unread == 1) {
                    Mail.mail.sendPlayerMessage(player,
                                                Language.getInboxNewSingular().replaceAll("%numMessages", "" + unread));
                } else {
                    Mail.mail.sendPlayerMessage(player,
                                                Language.getInboxNewPlural().replaceAll("%numMessages", "" + unread));
                }
            } else {
                Mail.mail.sendPlayerMessage(player, Language.getInboxEmpty());
            }
