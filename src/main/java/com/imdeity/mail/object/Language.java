...
public class Language {
    ...
    public void loadDefaults() {
        ...
        if (!lang.contains("mail.inbox.new.singular")) {
            lang.set("mail.inbox.new.singular", "&fYou have &a%numMessages new message &fin your mailbox!");
        }

        if (!lang.contains("mail.inbox.new.plural")) {
            lang.set("mail.inbox.new.plural", "&fYou have &a%numMessages new messages &fin your mailbox!");
        }
        ...
        if (!lang.contains("mail.delete.singular")) {
            lang.set("mail.delete.singular", "&fSuccessfully deleted message number &a%messageNumber!");
        }

        if (!lang.contains("mail.delete.plural")) {
            lang.set("mail.delete.plural", "&fSuccessfully deleted all your mail");
        }

        if (!lang.contains("mail.sent")) {
            lang.set("mail.sent", "&fYour message has been sent to &a%messageReceiver - %messageMessage");
        }

        if (!lang.contains("mail.message.long")) {
            lang.set("mail.message.long",
                     "&7[%messageLocalIndex] &f%messageSender: &7%messageLongMessage &8[%messageSendDate&8]");
        }

        if (!lang.contains("mail.message.short")) {
            lang.set("mail.message.short", "&7[%messageLocalIndex] &f%messageSender: &7%messageShortMessage");
        }

        if (!lang.contains("mail.error.permission")) {
            lang.set("mail.error.permissions", "&cYou do not have permissions to do that");
        }

        if (!lang.contains("mail.error.help")) {
            lang.set("mail.error.help", "&eInvalid Syntax, use \"/mail ?\" for help.");
        }

        if (!lang.contains("mail.error.invalid_selected")) {
            lang.set("mail.error.invalid_selected", "&cThat message doesn't exist.");
        }

        if (!lang.contains("mail.error.not_long_enough")) {
            lang.set("mail.error.not_long_enough", "&cYour message has to be longer then &4%messageLength words");
        }
        ...
        if (!lang.contains("mail.command.command")) {
            lang.set("mail.command.command", "&3%command: &b%explanation");
        }
