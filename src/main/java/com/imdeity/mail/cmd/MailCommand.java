...
public class MailCommand implements CommandExecutor {
    ...
    private void sendCommands(Player player) {
        ...
        if (player.hasPermission("mail.admin")) {
            ...
            output.add(this.formatCommands("/mail read <player> [num]", "Opens up the specified message."));
            output.add(this.formatCommands("/mail delete [num/*]", "Removes the specified message from your inbox."));
            output.add(this.formatCommands("/mail reload", "Reloads the language and config files"));
        } else {
            output.add(this.formatCommands("/mail", "Checks Inbox."));
            output.add(this.formatCommands("/mail write [player] [message]",
                                           "Sends a message to the specified person."));
            output.add(this.formatCommands("/mail read [num]", "Opens up the specified message."));
            output.add(this.formatCommands("/mail delete [num/*]", "Removes the specified message from your inbox."));
        }
    ...
    private String formatCommands(String command, String explanation) {
        String tmp = Language.getMailCommandMessage().replaceAll("%command", command).replaceAll("%explanation",
                         explanation);
    ...
    public void parseCommand(Player player, String[] split) throws SQLException {
        if (split.length == 0) {
            ...
            sendCommands(player);
        } else if (split[0].equalsIgnoreCase("read") || split[0].equalsIgnoreCase("r")) {
            readCommand(player, split);
        } else if (split[0].equalsIgnoreCase("delete") || split[0].equalsIgnoreCase("d")) {
            closeCommand(player, split);
        } else if (split[0].equalsIgnoreCase("write") || split[0].equalsIgnoreCase("w")
                   || split[0].equalsIgnoreCase("send") || split[0].equalsIgnoreCase("s")) {
            writeCommand(player, split);
        } else if (split[0].equalsIgnoreCase("reload")) {
            Mail.mail.reloadConfig();
            Mail.mail.sendPlayerMessage(player, "Reloaded Config.yml and Language.yml");
    ...
    private void closeCommand(Player player, String[] split) throws SQLException {
    ...
    private void writeCommand(Player player, String[] split) throws SQLException {
        if (player.hasPermission("mail.player.write")) {
            ...
            if (split.length <= 5) {
                plugin.sendPlayerMessage(player, Language.getMailNotLongEnough().replaceAll("%messageLength", "3"));
            ...
            plugin.sendPlayerMessage(player,
                                     Language.getMailSent().replaceAll("%messageReceiver",
                                         receiver).replaceAll("%messageMessage", message));
        } else {
            plugin.sendPlayerMessage(player, Language.getMailErrorPermission());
        }
    }
